import { mountJournal } from './prayer-journal.js';
/* ============================================================
...
const UI = {
  zh: {
    ...
    encourageLabel: '今日行动', safetyLabel: '请立刻寻求帮助',
  ...
  en: {
    ...
    encourageLabel: 'Today’s action', safetyLabel: 'Please reach out now',
...
const GUIDED = [
  { zh: '我感到孤单，想有人陪我祷告', en: 'I feel lonely and would like prayer' },
  { zh: '我正在失业和求职中', en: 'I am unemployed and looking for work' },
...
export function mountPrayerCare(container, opts) {
  ...
  const journalHost = document.createElement('div'); container.append(journalHost);
  const journal = mountJournal(journalHost, () => detectLangFrom(opts), (q, context) => pray(q, context));
  ...
  function pray(q, context = null) {
    followContext = context;
    if (!context) journal.reset();
    ...
    requestAIPrayer(request, lang, { signal: ctrl ? ctrl.signal : undefined, context })
      .then((data) => {
        if (inflight !== ctrl) return;          // superseded by a newer request
        inflight = null;
        renderAIPrayerBoard(answer, data, lang, boardOpts());
        journal.attach(answer, data, request, lang);
      })
      .catch((err) => {
        if (err && err.code === 'aborted') return;
        if (inflight !== ctrl) return;
        inflight = null;
        renderPrayerState(answer, 'error', lang, { onRetry: () => pray(request, context) });
      });
  }
  ...
  function renderChrome() {
    ...
    journal.render();
    if (lastRequest && lastMode) pray(lastRequest, followContext);
  }
