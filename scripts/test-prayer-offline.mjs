...
const env={ANTHROPIC_API_KEY:'test-only-not-a-real-key', PRAYER_RATE_LIMITER:{limit:async()=>({success:true})}, SCRIPTURE_ASSETS:{fetch:async()=>new Response(JSON.stringify({'35':'Jesus wept.'}))}};
...
const modelData={...valid,verses:[{book:'JHN',chapter:11,verseStart:35}]};
const upstream=(data=modelData,stop='end_turn')=>new Response(JSON.stringify({stop_reason:stop,content:[{type:'text',text:JSON.stringify(data)}]}));
...
test('unconfigured service explicitly fails',async()=>assert.equal((await worker.fetch(request(),{})).status,503));
...
test('malformed, incomplete and invalid-reference responses fail closed',async()=>{
 for(const data of [{...modelData,verses:[]},{...modelData,understanding:''},{...modelData,crisis:'false'},{...modelData,crisis:true,safety:''},{...modelData,verses:[{book:'JHN',chapter:99,verseStart:1}]}]){
 globalThis.fetch=async()=>upstream(data);assert.equal((await worker.fetch(request(),env)).status,502);
 }
});
test('truncated completion rejected even if JSON looks complete',async()=>{globalThis.fetch=async()=>upstream(modelData,'max_tokens');assert.equal((await worker.fetch(request(),env)).status,502);});
